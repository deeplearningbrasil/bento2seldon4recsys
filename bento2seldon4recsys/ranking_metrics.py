"""
Fork from: https://gist.github.com/bwhite/3726239/7e4dd612254e45fa275f9ddd9eb4af384a02794c
...
def prediction_coverage(predicted: List[List[Any]], catalog: List[Any]) -> float:
    """
...
def personalization(predicted: List[List[Any]]) -> float:
    """
    Forked from https://github.com/statisticianinstilettos/recmetrics
    ...
    rec_matrix_sparse = make_rec_matrix(predicted_array)
    # calculate similarity for every user's recommendation list
    similarity = cosine_similarity(X=rec_matrix_sparse, dense_output=False)

    del predicted, predicted_array, rec_matrix_sparse

    data = sp.triu(similarity, k=1).data
    if len(data) > 0:
        # calculate average similarity
        mean_similarity = float(np.mean(data))
        return 1 - mean_similarity
    else:
        return 1


def _get_predicted_at_k(predicted: List[List[Any]], k: int) -> List[List[Any]]:
    return [p[:k] for p in predicted]


def prediction_coverage_at_k(
    predicted: List[List[Any]], catalog: List[Any], k: int
) -> float:
    return prediction_coverage(_get_predicted_at_k(predicted, k), catalog) / 100


def personalization_at_k(predicted: List[List[Any]], k: int) -> float:
